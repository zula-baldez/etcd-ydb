...
func runTestCase(t *testing.T, client *etcd.Client, tc TestCase) {
	...
	if _, isDelete := tc.request.(*etcd.DeleteRequest); isDelete {
		assert.Equal(t, true, cmp.Equal(tc.response.(*etcd.DeleteResponse).PrevKvs, actual.(*etcd.DeleteResponse).PrevKvs, cmpopts.SortSlices(func(x, y *etcd.KeyValue) bool {
